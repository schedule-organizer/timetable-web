(function () {
  ...
  const isProfilePage = /\/profile-management\.html$/.test(path);
  const isPreAuthPage = path.endsWith("/docs/ux-mockups/index.html") || /\/(auth-login|auth-register|auth-complete-registration)\.html$/.test(path);
  const sessionRole = (body.dataset.sessionRole || "").toLowerCase();

  if (!isModerator && !isSystemAdmin && !isDailyOperations && !isNotifications && !isTimetableAccess && !isProfilePage && !sessionRole) {
    return;
  }
